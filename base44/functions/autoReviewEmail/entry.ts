...
// Also callable directly with order_id for manual triggers
Deno.serve(async (req) => {
  try {
    ...
    // If called by entity automation, extract from event payload
    if (!order_id && body.event?.entity_id) {
      order_id = body.event.entity_id;
    }
    if (!order_id && body.data?.id) {
...
<head><meta charset="utf-8"><style>
  ...
  .tracking { background: #f9f9f9; border-radius: 10px; padding: 12px 16px; margin: 12px 0; display: inline-block; width: 100%; box-sizing: border-box; }
</style></head>
<body>
<div class="container">
  <div class="header">
  ...
  <div class="body">
    ...
    <p style="text-align:center; font-size:13px; color:#aaa;">Solo toma 30 segundos • Sin registro adicional</p>
  </div>
  <div class="footer">
