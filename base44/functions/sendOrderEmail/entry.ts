...
Deno.serve(async (req) => {
  try {
    ...
    const itemsList = (order.items || []).map(i => `• ${i.name} x${i.quantity} — $${(i.price || 0).toFixed(2)}`).join('\n');
    ...
    if (event_type === 'new_order') {
          ...
          <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #fff;">
            ...
            <div style="padding: 30px; background: #fff;">
              ...
              <div style="background: #f9f9f9; border-radius: 12px; padding: 20px; margin: 20px 0;">
                ...
                <p style="color: #333; font-weight: bold; font-size: 18px; margin: 0;">Total: $${(order.total || 0).toFixed(2)}</p>
              </div>
      ...
      const adminSubject = `🍓 Nuevo Pedido #${order.tracking_code} — $${(order.total || 0).toFixed(2)}`;

      if (adminEmail) {
        await base44.asServiceRole.integrations.Core.SendEmail({ to: adminEmail, subject: adminSubject, body: adminBody }).catch(() => {});
      }
      ...
      const templateKey = `email_template_${order.status}_es`;
      const customTemplate = settings[templateKey];

      if (customTemplate) {
        // Custom plain-text-style template
        subject = `🍓 Tu pedido está: ${statusLabel} — #${order.tracking_code}`;
        const messageText = applyTemplate(customTemplate);
        body_html = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="padding: 24px; background: #fff; text-align: center;">
              <div style="font-size: 50px; margin: 16px 0;">${statusEmoji[order.status] || '🍓'}</div>
              <p style="font-size: 22px; font-weight: bold; color: #E8294A;">${statusLabel}</p>
              <p style="color: #888;">Pedido #${order.tracking_code}</p>
              <div style="background: #FDE8EC; border-radius: 12px; padding: 16px; margin: 20px 0; text-align: left;">
                <p style="color: #555; line-height: 1.6; margin: 0;">${messageText}</p>
              </div>
            </div>
          </div>`;
      } else {
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="padding: 24px; background: #fff; text-align: center;">
              ...
              <div style="background: #FDE8EC; border-radius: 16px; padding: 24px; margin: 20px 0;">
                <p style="font-size: 40px; margin: 0;">${statusEmoji[order.status] || '🍓'}</p>
